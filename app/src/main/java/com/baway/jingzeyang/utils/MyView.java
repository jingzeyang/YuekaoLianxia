...
import com.baway.jingzeyang.HomeActivoty;
...
public class MyView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //给大圆路径设置参数
        greatPath.addCircle(getWidth() / 2, getHeight() / 2, 200, Path.Direction.CW);
        //将path设置进入Region
        ...
        //给小圆路径设置参数
        smallPath.addCircle(getWidth() / 2, getHeight() / 2, 100, Path.Direction.CW);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            } else if (greatRegion.contains(x, y)) {

                Toast.makeText(getContext(), "圆环内", Toast.LENGTH_SHORT).show();
            } else {

                Intent it = new Intent(getContext(), HomeActivoty.class);
